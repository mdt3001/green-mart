    ...
    return (
        <div className="text-slate-500 mb-40">
            ...
            <form onSubmit={handleAddCoupon} className="max-w-2xl text-sm">
                <h2 className="text-2xl mb-4">Thêm <span className="text-slate-800 font-medium">Mã Giảm Giá</span></h2>
                    ...
                    <div className="flex items-center gap-3">
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                                ...
                                onChange={(e) => setNewCoupon({ ...newCoupon, for_new_user: e.target.checked })}
                            />
                    ...
                    <div className="flex items-center gap-3">
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                                ...
                                onChange={(e) => setNewCoupon({ ...newCoupon, for_member: e.target.checked })}
                            />
                            <div className="w-11 h-6 bg-slate-300 rounded-full peer peer-checked:bg-green-600 transition-colors"></div>
                            <span className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-5"></span>
                        </label>
                    ...
                    <div className="flex items-center gap-3">
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input 
                                ...
                                onChange={(e) => setNewCoupon({ ...newCoupon, is_public: e.target.checked })}
                            />
                            <div className="w-11 h-6 bg-slate-300 rounded-full peer peer-checked:bg-green-600 transition-colors"></div>
                            <span className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-5"></span>
                        </label>
                ...
                {!newCoupon.is_public && (
                    <div className="mt-4">
                        <label className="block text-sm font-medium mb-2">Chọn cửa hàng</label>
                        <div className="max-h-40 overflow-y-auto border border-slate-200 rounded-md p-2">
                            {stores.map(store => (
                                <label key={store.id} className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded cursor-pointer">
                                    <input
                                        type="checkbox"
                ...
                <button type="submit" className="mt-5 py-2 px-10 rounded bg-green-600 text-white hover:bg-green-700 active:scale-95 transition">
                    Thêm Mã Giảm Giá
                </button>
            </form>
            ...
            {/* Coupons List */}
            <div className="mt-14">
                <h2 className="text-2xl mb-4">Danh Sách <span className="text-slate-800 font-medium">Mã Giảm Giá</span></h2>
                    ...
                    <table className="min-w-full bg-white text-sm">
                        <thead className="bg-slate-50">
                            <tr>
                                ...
                                <th className="py-3 px-4 text-left font-semibold text-slate-600">Loại</th>
                                <th className="py-3 px-4 text-left font-semibold text-slate-600">Cửa hàng</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {coupons.map((coupon) => (
                                <tr key={coupon.code} className="hover:bg-slate-50">
                                    ...
                                    <td className="py-3 px-4 text-green-600 font-semibold">{coupon.discount}%</td>
                                    <td className="py-3 px-4 text-slate-800">
                                        {coupon.expires_at ? format(new Date(coupon.expires_at), 'dd/MM/yyyy') : 'Vô thời hạn'}
                                    </td>
                                    <td className="py-3 px-4">
                                        <div className="flex gap-1 flex-wrap">
                                            {coupon.for_new_user && <span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">Mới</span>}
                                            {coupon.for_member && <span className="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded">VIP</span>}
                                            {coupon.is_public && <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">Công khai</span>}
                                        </div>
                                    </td>
                                    <td className="py-3 px-4">
                                        <button 
                                        ...
                                        >
                                            <StoreIcon size={16} />
                                            <span className="text-xs">({coupon.stores?.length || 0})</span>
                                        </button>
                                    </td>
                                    <td className="py-3 px-4">
                                        <button 
            ...
            {showAssignModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                        <h3 className="text-xl font-semibold mb-4">Gán Cửa Hàng - {selectedCoupon?.code}</h3>
                        
                        <div className="max-h-64 overflow-y-auto mb-4">
                            {stores.map(store => (
                                <label key={store.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded cursor-pointer">
                                    <input
                                        type="checkbox"
