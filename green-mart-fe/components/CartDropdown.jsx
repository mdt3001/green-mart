...
import { updateCartItem, removeCartItem, fetchCart } from "@/lib/redux/features/cart/cartSlice";
                  ...
                  <div key={item.id} className={`p-3 sm:p-4 hover:bg-gray-50 transition-colors ${
                    ...
                    <div className="flex gap-2 sm:gap-3 relative">
                      ...
                      <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                        {(item.product?.images?.[0] || item.images?.[0] || item.image) ? (
                          <Image
                            src={item.product?.images?.[0] || item.images?.[0] || item.image}
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          href={`/product/${item.product?.id || item.id}`}
                        ...
                        <p className="text-xs sm:text-sm text-green-600 font-semibold mt-0.5 sm:mt-1">
                          {formatPrice(parseFloat(item.product?.price || item.price) || 0)}
                        </p>
                      ...
                      <button
                        ...
                        title="Xóa"
                      >
                        <Trash2 size={14} className="sm:w-4 sm:h-4 text-gray-400 group-hover:text-red-600 transition-colors" />
                      </button>
                    </div>
                  </div>
                  );
          ...
          {items.length > 0 && (
            <div className="p-3 sm:p-4 bg-gray-50 border-t border-gray-200">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <span className="text-sm sm:text-base text-gray-600">Tạm tính:</span>
